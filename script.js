...
// Format R:R ratio (numeric only, e.g. "2.00")
function formatRR(value) {
  const num = Number(value) || 0;
  return num.toFixed(2);
}

// Format R:R with colon and optional risk/reward amounts: "1 : 2.00 (Risk $X / Reward $Y)"
function formatRRWithColon(trade) {
  ...
  let riskStr = "";
  let rewardStr = "";
  if (entry && sl && tp && lotSize && Number.isFinite(lotSize)) {
    ...
    riskStr = formatCurrency(riskAmount);
    rewardStr = formatCurrency(rewardAmount);
  }
  const ratio = (rr && Number.isFinite(rr)) ? rr.toFixed(2) : "0.00";
  const colonPart = "1 : " + ratio;
  if (riskStr && rewardStr) return colonPart + " (Risk " + riskStr + " / Reward " + rewardStr + ")";
  return colonPart;
}
...
function renderDashboard() {
  ...
  sorted.slice(0, 50).forEach((trade) => {
    ...
    tr.innerHTML = `
      ...
      <td>${escapeHTML(formatRRWithColon(trade))}</td>
    `;
...
function renderTradeDetail(id) {
  ...
  wrapper.innerHTML = `
    ...
    <div class="trade-detail-item">
      ...
      &nbsp;&nbsp;P/L: ${formatCurrency(trade.pl)} &nbsp;&nbsp;R:R ${escapeHTML(formatRRWithColon(trade))}
    </div>
...
function renderPerformance() {
  ...
  // Pie chart: Total Profit, Total Loss, Number of Trades (different colors)
  if (chartSvg && chartTooltip) {
    ...
    // Scale trades for pie (so all three segments are visible)
    const scale = Math.max(totalProfit, totalLoss, 1) / Math.max(tradeCount, 1);
    const tradesValue = tradeCount * scale;

    const segments = [
      { key: "profit", value: Math.max(0, totalProfit), label: "Total Profit", color: "#22a86e", fmt: (v) => formatCurrency(v) },
      { key: "loss", value: Math.max(0, totalLoss), label: "Total Loss", color: "#d9304c", fmt: (v) => formatCurrency(v) },
      { key: "trades", value: tradesValue, label: "Trades", color: "#5b7cff", fmt: () => tradeCount + " trade(s)" },
    ].filter((s) => s.value > 0);

    const totalSum = segments.reduce((sum, s) => sum + s.value, 0);
    ...
    const legendContainer = document.querySelector(".performance-chart-legend");
    if (legendContainer) {
      legendContainer.innerHTML = segments
        .map(
          (s) =>
            `<button type="button" data-segment="${s.key}">${s.key === "trades" ? "Trades: " + tradeCount : s.label + " " + (s.key === "profit" ? formatCurrency(totalProfit) : formatCurrency(totalLoss))}</button>`
        )
        .join("");
    }

    if (segments.length === 0 || totalSum <= 0) {
      ...
      chartTooltip.textContent = "No trades in range. Change trader or dates.";
    } else {
      if (bgCircle) bgCircle.setAttribute("fill", "transparent");
      let cumulative = 0;
      segments.forEach((seg) => {
        const startAngle = (cumulative / totalSum) * 2 * Math.PI;
        const endAngle = ((cumulative + seg.value) / totalSum) * 2 * Math.PI;
        ...
        path.setAttribute("aria-label", seg.label);
        chartSvg.appendChild(path);
      });

      const segmentMap = {};
      segments.forEach((s) => (segmentMap[s.key] = s));

      function setTooltip(key) {
        const seg = segmentMap[key];
        if (!seg) return;
        const val = key === "profit" ? totalProfit : key === "loss" ? totalLoss : tradeCount;
        const text = key === "trades" ? `Trades: ${val}` : `${seg.label}: ${seg.fmt(val)}`;
      ...
      chartTooltip.textContent = "Hover over chart or legend.";
    }
  }
}
